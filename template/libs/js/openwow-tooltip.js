if (typeof $oWoW == "undefined") {
    $oWoW = {
        cdnUrl: '//cdn.openwow.com',
        expUrl: '.openwow.com'
    };
}
if (typeof $oWoWTooltip == "undefined") {
    var $oWoWTooltip = new function() {
        var external = true;
        ...
        var
            ...
            currentType, currentId, currentLocale, currentDomain, currentParams, currentExpansion, currentA, cursorX, cursorY, mode = 0,
            eventAttached = false,
            npcs = {},
            objects = {},
            items = {},
        ...
        function init() {
            if (external) {
                ...
                script.src = $oWoW.cdnUrl + "/script/global.js";
        ...
        function attachEvent() {
            ...
            eventAttached = true;
            $oWoW.aE(document, "mouseover", onMouseOver);
        }
        ...
        this.init = function() {
            if (external) {
                $oWoW.ae(headtag, $oWoW.ce("link", {
                    type: "text/css",
                    href: $oWoW.cdnUrl + "/style/global.css",
                    rel: "stylesheet"
                }));
            }
            attachEvent();
            onDOMReady(function() {
                if (typeof openwowTooltips != "undefined") {
        ...
        function scanElement(t, e) {
            ...
            if (method.type & 1) {
                ...
                if (t.href.indexOf("http://") == 0 || t.href.indexOf("https://") == 0) {
                    i0 = 1;
                    url = t.href.match(/^https?:\/\/(www|classic|tbc|wotlk|cata|mop|wod)?\.?openwow\.com\/.?(item|quest|spell|achievement|npc|object)=([0-9]+)/);
                } else {
            ...
            if (url) {
                ...
                locale = $oWoW.getLocaleFromDomain(domain);
                expansion = $oWoW.getExpansionFromDomain(domain);
                ...
                var
                    type = $oWoW.getIdFromTypeName(url[i1]),
                    typeId = url[i2];
                display(type, typeId, locale, params, expansion);
                if (e || typeof openwowTooltips == "undefined") {
                    return;
                }
                ...
                var timeout = function(t) {
                    ...
                    if (openwowTooltips.icons && data.icon && (type == TYPE_ITEM || type == TYPE_ACHIEVEMENT || type == TYPE_SPELL)) {
                        if (external) {
                            ...
                            if (openwowTooltips.iconsize) {
                                t.innerHTML = t.innerHTML + '<img style="width: ' + openwowTooltips.iconsize + 'px; height: ' + openwowTooltips.iconsize + 'px;" src="//cdn.openwow.com/' + domain + '/icons/tiny/' + data.icon.toLocaleLowerCase() + '.gif">';
                            } else {
                                t.innerHTML = t.innerHTML + '<img style="width:15px; height:15px;" src="//cdn.openwow.com/' + domain + '/icons/tiny/' + data.icon.toLocaleLowerCase() + '.gif">';
                            }
                        } else {
                            t.className += " icontinyl";
                            t.style.background = "url(//cdn.openwow.com/" + domain + "/icons/tiny/" + data.icon.toLocaleLowerCase() + ".gif)";
                        }
                    }
                    if (openwowTooltips.rename) {
                    ...
                    if (openwowTooltips.qualitycolor) {
                        if (type == TYPE_ITEM) {
                            if (typeof openwowTooltips.overridecolor != "undefined" && typeof openwowTooltips.overridecolor.items != "undefined" && openwowTooltips.overridecolor.items) {
                                t.style.color = openwowTooltips.overridecolor.items;
                            } else {
                                t.className += " q" + data.quality;
                            }
                        }
                        if (typeof openwowTooltips.overridecolor != "undefined") {
                            if (type == TYPE_SPELL) {
                                if (typeof openwowTooltips.overridecolor.spells != "undefined" && openwowTooltips.overridecolor.spells) {
                                    t.style.color = openwowTooltips.overridecolor.spells;
                                }
                            }
                            if (type == TYPE_ACHIEVEMENT) {
                                if (typeof openwowTooltips.overridecolor.achievements != "undefined" && openwowTooltips.overridecolor.achievements) {
                                    t.style.color = openwowTooltips.overridecolor.achievements;
                                }
                            }
                            if (type == TYPE_QUEST) {
                                if (typeof openwowTooltips.overridecolor.quests != "undefined" && openwowTooltips.overridecolor.quests) {
                                    t.style.color = openwowTooltips.overridecolor.quests;
                                }
                            }
                            if (type == TYPE_NPC) {
                                if (typeof openwowTooltips.overridecolor.npcs != "undefined" && openwowTooltips.overridecolor.npcs) {
                                    t.style.color = openwowTooltips.overridecolor.npcs;
                                }
                            }
                            if (type == TYPE_OBJECT) {
                                if (typeof openwowTooltips.overridecolor.objects != "undefined" && openwowTooltips.overridecolor.objects) {
                                    t.style.color = openwowTooltips.overridecolor.objects;
                                }
                            }
                        }
                    }
        ...
        function onMouseMove(a) {
            a = $oWoW.evt(a);
            updateCursorPos(a);
            $oWoW.Tooltip.move(cursorX, cursorY, 0, 0, CURSOR_HSPACE, CURSOR_VSPACE);
        }
        ...
        function request(type, id, locale, stealth, params, expansion) {
            ...
            var xpac = $oWoW.getDomainFromExpansion(expansion);
            var url = "//" + xpac + $oWoW.expUrl + "/";
            $oWoW.ajaxRequest(url + LOOKUPS[type][1] + "=" + id + "&power=true" + p);
        }
        ...
        function showTooltip(html, icon, map, spellData, html2, expansion) {
            ...
            if (mode == 1) {
                $oWoW.Tooltip.setIcon(null);
                $oWoW.Tooltip.show(currentA, html, null, null, null, html2);
            } else {
                $oWoW.Tooltip.setIcon(icon, expansion);
                $oWoW.Tooltip.showAtXY(html, cursorX, cursorY, CURSOR_HSPACE, CURSOR_VSPACE, html2);
            }
        }
        ...
        this.setParent = function(newParent) {
            $oWoW.Tooltip.reset();
            $oWoW.Tooltip.prepare(newParent);
        };
        if (external) {
