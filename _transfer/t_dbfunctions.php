...
function CanOrNoTransferPlayer($DBHost, $DBUser, $DBPassword, $CharactersDB, $AccountDB, $AccountID) {
    ...
    _SelectDB($CharactersDB, $connection);
    // check if account has too many characters
...
function CanOrNoTransferServer($DBHost, $DBUser, $DBPassword, $AccountDB, $RealmID, $GMLevel, $CharacterDB) {
    $connection = mysql_connect($DBHost, $DBUser, $DBPassword) or die(mysql_error());
    _SelectDB($AccountDB, $connection);
    ...
    if ($REVIEWER_ID < 0) {
        ...
        // security check to avoid character list bug bugs
        _SelectDB($CharacterDB, $connection);
        $query = mysql_query("SELECT COUNT(*) FROM `characters` WHERE `account` = " . $REVIEWER_ID . ";", $connection) or die(mysql_error());
        $result = mysql_fetch_array($query);

        if ($result[0] >= 500) { // 10 should be the correct value
            mysql_close($connection);
            return -1;
        }

        // AZTH CUSTOM CHECK
        $query = mysql_query("SELECT COUNT(*) FROM character_transfer WHERE GUID IN (SELECT guid FROM characters WHERE account = " . $REVIEWER_ID . ");", $connection) or die(mysql_error());
        $result = mysql_fetch_array($query);

        if ($result[0] >= 500) { // maybe 2 is better
            mysql_close($connection);
            return -1;
        }
...
function _CheckRiding($SKILL, $CUR, $connection, $GUID, $LEVEL) {
    ...
    switch ($SKILL) {
        ...
        case "ВЕРХОВАЯ ЕЗДА":   // ruRU
            switch ($CUR) {
                case 75: $SpellID = 33388;
                    break;
                case 150: $SpellID = 33391;
                    break;
                case 225: $SpellID = 34090;
                    break;
